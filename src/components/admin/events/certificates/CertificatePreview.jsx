...
export default function CertificatePreview({ template, onClose, onUpdatePositions, type, isTeam = false }) {
  const [selectedField, setSelectedField] = useState(null);
  const [positions, setPositions] = useState(template.positions || {});
  const [previewMode, setPreviewMode] = useState(false);
  const imageRef = useRef(null);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-auto p-4">
          <div className="relative inline-block">
            ...
            {!previewMode && (
              <div className="top-2 left-2 space-y-1 bg-white/90 dark:bg-gray-800/90 p-2 rounded-md backdrop-blur-sm">
