...
import { Calendar, Award, Download, CheckCircle, Users, Trophy, X} from 'lucide-react';
...
function PaidEventCard({ registration }) {
  ...
  const checkEventType = async () => {
    try {
      const subeventsResponse = await api.get(`/subevents/${registration.event_id}`);
  ...
  const checkTeamStatus = async () => {
    try {
      const response = await api.get(`/teams/my-team/${registration.event_id}/${registration.subevent_id}`);
      if (response.data) {
  ...
  const fetchTeamLeaderboard = async () => {
    try {
      const [teamResponse, leaderboardResponse] = await Promise.all([
        api.get(`/teams/my-team/${registration.event_id}/${registration.subevent_id}`),
        api.get(`/teams/leaderboard/${registration.event_id}/${registration.subevent_id}`)
      ]);
      ...
      if (myTeamData) {
        const myTeamEntry = leaderboardResponse.data.find(entry => entry.team_id === myTeamData.id);
        if (myTeamEntry) {
          const rank = leaderboardResponse.data.findIndex(entry => entry.team_id === myTeamData.id) + 1;
          setTeamRank(rank);
        }
      }
    } catch (error) {
  ...
  const fetchIndividualLeaderboard = async () => {
    try {
      ...
      const userEntry = response.data.find(entry => entry.student_id === registration.student_id);
      if (userEntry) {
        const rank = response.data.findIndex(entry => entry.student_id === registration.student_id) + 1;
  ...
  return (
    <>
              ...
              {isTeamEvent ? (
                teamRank && teamRank <= 3 && (
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                    teamRank === 1 ? 'bg-yellow-100 text-yellow-800' :
                    teamRank === 2 ? 'bg-gray-100 text-gray-800' :
                    'bg-amber-100 text-amber-800'
                  }`}>
                    <Trophy className="h-3 w-3 mr-1" />
                    {teamRank}{getRankSuffix(teamRank)} Place Team
                  </span>
                )
              ) : (
                individualRank && individualRank <= 3 && (
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                    individualRank === 1 ? 'bg-yellow-100 text-yellow-800' :
                    individualRank === 2 ? 'bg-gray-100 text-gray-800' :
                    'bg-amber-100 text-amber-800'
                  }`}>
                    <Trophy className="h-3 w-3 mr-1" />
                    {individualRank}{getRankSuffix(individualRank)} Place
                  </span>
                )
              )}
            </div>
        ...
        <div className="flex space-x-3 mt-4">
          ...
          >
            ...
            {downloading ? 'Downloading...' : `Download ${
              isTeamEvent ? 
                (teamRank && teamRank <= 3 ? 'Merit' : 'Participation') : 
                (individualRank && individualRank <= 3 ? 'Merit' : 'Participation')
            } Certificate`}
          </button>
        </div>
      </div>
      ...
      {/* Leaderboard Modal */}
      {showWinners && (
        <div className="fixed inset-60 z-50">
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            {/* Background overlay */}
            <div className="fixed inset-0 transition-opacity bg-black/50 backdrop-blur-sm" onClick={() => setShowWinners(false)} />
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="font-bold">{winner.student_name}</span>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="font-bold">{winner.score} points</div>
                          <span className={`text-sm ${
                            winner.rank === 1 ? 'text-yellow-600' :
                            winner.rank === 2 ? 'text-gray-600' :
                            'text-amber-600'
                          }`}>
                            {winner.rank}{getRankSuffix(winner.rank)} Place
                          </span>
                        </div>
                      </div>
                    </div>
